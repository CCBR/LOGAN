...
workflow ALIGN {
    ...
    if (params.indelrealign){
        ...
        bambyinterval=alignment_out.combine(splitinterval.out.flatten())
...
workflow VC {
    ...
    if ("octopus" in call_list){
        octopus_in = bambyinterval
            | octopus_tn
            | bcftools_index_octopus
            | groupTuple()
            | map{samplename,vcf,vcfindex ->
                tuple(samplename, vcf.toSorted{it->(it.name =~ /${samplename}_(.*).octopus.vcf.gz/)[0][1].toInteger()}, vcfindex, "octopus", "")
            }
            | view
